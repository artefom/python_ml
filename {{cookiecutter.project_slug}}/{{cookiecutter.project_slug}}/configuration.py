...
class EnvConfigParser(ConfigParser):
    ...
    def _env_var_name(section, key):
        return '{{cookiecutter.env_prefix}}__{S}__{K}'.format(S=section.upper(), K=key.upper())

    def _get_env_var_option(self, section, key):
        # must have format {{cookiecutter.env_prefix}}__{SECTION}__{KEY} (note double underscore)
...
def get_config(home_dir):
    if '{{cookiecutter.env_prefix}}_CONFIG' not in os.environ:
        return os.path.join(home_dir, '{{cookiecutter.project_slug}}.cfg')
    return expand_env_var(os.environ['{{cookiecutter.env_prefix}}_CONFIG'])
...
DEFAULT_CONFIG = _read_default_config_file('default_{{cookiecutter.project_slug}}.cfg')

if not os.path.isfile(CONFIG_PATH):
    log.info(
        'Creating new {{cookiecutter.project_slug}} config file in: %s',
        CONFIG_PATH
    )
